...
const Home = () => {
  ...
  const createClass = async () => {
    ...
    await axios.post('http://localhost:3113/create_class', {
      userId, title
    });
  }
  ...
  const joinClass = async () => {
    ...
    await axios.post('http://localhost:3113/add_class', {
      userId, classId
    });
  };

  const fetchClasses = async () => {
    const res = await axios.get('http://localhost:3113/get_classes/' + userId);
